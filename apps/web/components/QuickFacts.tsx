...
/** 부모가 3초 안에 훑는 6칸: 연령·아동 요금·보호자·양말·예약·주차. 모르면 회색 "미확인". */
  ...
  return (
    <ul className={`grid gap-2 ${compact ? "grid-cols-6" : "grid-cols-3"}`} aria-label="핵심 이용 정보">
      {facts.map((f) => {
        ...
        const inner = (
          <>
            <Icon size={compact ? 16 : 22} className={known ? "text-neutral-900" : "text-neutral-300"} />
        ...
        const cls = `flex flex-col items-center justify-center rounded-xl text-center ${compact ? "px-1 py-1.5" : "px-2 py-3"} ${
          known ? "border border-neutral-200 bg-white" : "border border-dashed border-neutral-200 bg-neutral-50"
        }`;
        return (
          <li key={f.key} title={f.sub ?? f.label}>
            {f.href ? (
              <a href={f.href} target="_blank" rel="noopener noreferrer" className={`${cls} border-emerald-300 hover:bg-emerald-50`}>
                {inner}
              </a>
            ) : (
