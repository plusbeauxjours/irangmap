...
- 후기는 저장하지 않고 **네이버 블로그·카페 검색 딥링크**로 보낸다(`linkouts`, 인스타그램 검색 링크는 결과가 나쁘어 제외): 쿼리 = 업소명(㈜ 제거) + 주소의 구·시·군/동 + "후기" — 지점 많은 브랜드의 다른 지점 글을 걸러주는 지역 힌트(`regionHint`). 2단계(네이버 검색 API로 최근 글 표시)는 키 발급 후.
