...
## 다음 개선 후보 (2026-09-20 저녁, 카카오맵 전환 직후)

1. **데이터 채움이 최우선** — 2,911곳 중 이용 정보 181곳. 사업자 클레임·이용자 제보(DB 필요 → Docker), 네이버 검색 API 2단계(최근 후기 표시, 네이버 개발자 키), 프랜차이즈 잔여 채널(챔피언 JSON 목록 111곳 전화, 타요키즈 양말 공지 URL 수정, 코코몽 1시간 딜레이 크롤).
2. **탐색 UX** — 지역(구·동) 검색, "내 위치" 버튼(geolocation), URL에 지도 상태·선택 업소 반영(공유·뒤로가기), 모바일 바텀시트.
3. **신뢰** — 폐업 신선도(영업상태 배지·"마지막 확인"), 폐업/오류 신고 버튼(제보 파이프라인과 함께), 출처 페이지.
4. **SEO·배포** — 업소 상세 SSR 페이지(`/venue/[id]`), 메타·OG, 운영 도메인 → 카카오 콘솔 Web 플랫폼에 추가, 약관·개인정보처리방침.
5. **분류 품질** — A004 식품접객업소(놀이방 있는 식당) 카테고리 노출 여부, generic 브랜드 단어 오탐 점검.
